...
client.on("messageCreate", async (messageCreate) => {
  ...
  webhook = webhook.find((x) => x.name === "Sparks" + number);

  if (!webhook) {
    webhook = await messageCreate.channel.createWebhook(`Sparks` + number, {
      avatar: client.user.displayAvatarURL({ dynamic: true }),
    });
  }

  await webhook.edit({
    name: messageCreate.member.nickname
      ? messageCreate.member.nickname
...
client.on("ready", () => {
  console.log(chalk.blue("NQN Bot") + chalk.cyan(` has been hosted`));
  client.user.setActivity("Sparks");
});
